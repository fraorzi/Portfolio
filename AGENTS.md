...
- Placement: each point stores an offset from its region anchor (`aInfo` = seed, region, span along thread, fade). `pointField.ts` derives `uAnchors[7]` from measured section tops (`src/lib/sceneProgress.ts`, refreshed by `useSceneLayout`) and the `.container-page` width. Desktop anchors follow the content container; bounds calculated from the shape and spine points keep them inside the camera's perspective with room for drift and parallax. Narrow viewports also reduce shape scale. Mobile shapes sit in the top band, with threads inset from the right edge; thread gutter coordinates account for both region anchors. The camera follows `window.scrollY`, keeping shapes attached to their sections.
